/*
  ...
  Created  date: 25/08/2023 (e.g. 31/07/2023)
...
//  Tictac
//
//  Created by Truong, Nguyen Tan on 25/08/2023.
//
...
struct Map1: View {
    ...
    @State public var highscore = UserDefaults.standard.integer(forKey: "highscore")
    ...
    //column layout
    let columnLayout = Array(repeating: GridItem(.fixed(90), spacing: 15, alignment: .center), count: 3)
    ...
    func checkWinner(list: [String], player: String) -> Bool {
        ...
        for sequence in winningSequences {
            ...
            for match in sequence {
                if list[match] == player {         //condition to determine who won and lose
                    matches += 1
                    if matches == 3 {
                        if (player == "X"){
                            print("\(name) has won!")
